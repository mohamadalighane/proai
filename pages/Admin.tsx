...
const Admin: React.FC<AdminProps> = ({ prompts, posts, courses, onDeletePrompt, onSavePrompt, onDeleteCourse, onSaveCourse, onNavigate }) => {
  ...
  const handlePromptSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const p: Prompt = {
      ...
      imageUrl: formData.get('imageUrl') as string || `https://picsum.photos/seed/${Math.random()}/600/800`,
  ...
  const handleCourseSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const c: Course = {
      ...
      title: formData.get('title') as string,
      description: formData.get('description') as string,
      thumbnail: formData.get('thumbnail') as string || `https://picsum.photos/seed/${Math.random()}/400/250`,
      ...
      {activeTab === 'prompts' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button 
              onClick={() => { setEditingPrompt(null); setShowPromptForm(true); }}
          ...
          {showPromptForm && (
            <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
              <div className="glass p-8 rounded-[2rem] w-full max-w-lg border border-white/10" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handlePromptSubmit} className="space-y-4">
                  ...
                  <input name="imageUrl" defaultValue={editingPrompt?.imageUrl} placeholder="لینک تصویر (اختیاری)" className="w-full bg-white/5 border border-white/10 rounded-xl p-3" />
                ...
                {prompts.map(p => (
                  <tr key={p.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 flex items-center gap-3">
                      <button onClick={() => { setEditingPrompt(p); setShowPromptForm(true); }} className="text-violet-400 hover:text-violet-300"><i className="fa-solid fa-pen"></i></button>
            ...
            <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
              <div className="glass p-8 rounded-[2rem] w-full max-w-lg border border-white/10" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleCourseSubmit} className="space-y-4">
                  ...
                  <input name="thumbnail" defaultValue={editingCourse?.thumbnail} placeholder="لینک کاور" className="w-full bg-white/5 border border-white/10 rounded-xl p-3" />
                  <div className="flex gap-2 pt-4">
                    <button type="submit" className="flex-1 bg-violet-600 py-3 rounded-xl font-bold">ذخیره</button>
            ...
            {courses.map(course => (
              <div key={course.id} className="glass p-4 rounded-2xl flex items-center justify-between border border-white/10">
                ...
                <div className="flex gap-3">
                  <button onClick={() => { setEditingCourse(course); setShowCourseForm(true); }} className="text-violet-400 p-2"><i className="fa-solid fa-pen"></i></button>
